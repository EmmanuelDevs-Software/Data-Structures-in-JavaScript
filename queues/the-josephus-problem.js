...
class Queue {
  ...
  dequeue() {
    ...
    this.size--;
    // get a reference to he node that was at the front of the list
    let result = this.front.data;
    if (this.front === this.tail) {
      // if there was just one node left then  there`s no more front or tal
      this.front = null;
      this.tail = null;
    } else {
...
// write a function that accepts an array of choices and returns
// and an integer n. You should cycle through the choices and
// starting at the beginning and counting up to N . Every
// time N is reached one choice should be removed. Cotinues
// this process counting to N and removing the choice at that
// count until only one choice is left . return that choice.
//[A,B,C,D,E,F] = n = 3 return B
//[A,B,C,D,E,F] = n = 4 return F
...
function josephus(choices, n) {
  ...
  // account for starting the 1 count at the secod item
